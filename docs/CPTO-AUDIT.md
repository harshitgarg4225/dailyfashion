...
| F8 | Biometric/passcode lock exists in `Settings` and in copy, with no UI row and no enforcement. J4 lists it as a feature. | P1 | DONE |
| F9 | `note` exists on `Entry` with no way to write one. Either build it or drop it from the schema. | P2 | DONE |
...
| U5 | Onboarding's buttons are both labelled "Next" with no sense of length or position beyond three dots. | P2 | DONE |
...
| D8 | No favicon/app icon refinement — the generated ring is a placeholder, not a mark. | P2 | DONE |
...
| # | Finding | Sev | Status |
|---|---|---|---|
| P1a | The killer insight ("you rate it highly and never wear it") needs 5 wears of one outfit, which needs the similarity matcher to cluster reliably. If clustering under-fires, the best card never appears. There is no instrumentation to know whether it fires. | P0 | DONE |
| P2a | Item-level findings ("your worst days correlate with those shoes") require lazy tags, which are optional and easy to skip. Realistically a minority of users ever reach n≥5 on an item. The colour card is the honest early substitute and should be foregrounded. | P1 | DONE |
...
| # | Finding | Sev | Status |
|---|---|---|---|
| S1 | Photos are stored unencrypted. **Deliberately not shipped** — see the note below. | P1 | WONT |
...
| S5 | No `Clear-Site-Data` on wipe. `deleteDatabase` handles our data; the service worker cache still holds the shell (harmless, but worth being exact about). | P2 | DONE |
...
| C3 | Signature computation and JPEG re-encode run on the main thread during capture, blocking the UI at exactly the moment the app promised to be fast. Belongs in a Worker. | P1 | DONE |
...
| C6 | `generateInsights` runs on every render of the insights screen, over the whole log. Needs memoising on entry count + dismissals. | P2 | DONE |
...
| M3 | No orientation handling on the camera; a landscape capture is stored rotated. | P1 | DONE |
| M4 | No `apple-touch-startup-image`, so the installed app shows a white flash on cold launch. | P2 | DONE |
| M5 | Tab bar does not account for landscape safe areas. | P2 | DONE |
| M6 | Camera preview uses `object-fit: cover`, which crops the frame — a full-length outfit shot can lose the shoes that a later insight depends on. | P1 | DONE |
...
## 9. The one thing deliberately not built

**S1 — encryption of photographs at rest.**

It sounds like a strict improvement and it is not, in this specific product.
There is no account and no server, so there is nowhere to recover a forgotten
passcode from. Encrypting the log would mean four forgotten digits permanently
destroy years of history — on a product whose entire value proposition is that
the history accumulates and becomes irreplaceable.

The threat encryption defends against here is an attacker with filesystem access
to an unlocked device profile. The threat the passcode lock defends against is
someone picking up your phone. The second is the realistic one for a log of
mirror selfies, and it is the one J4 actually describes.

So the lock ships and the encryption does not, until there is a real recovery
story — a key wrapped into the export archive, or a platform keychain that
survives a forgotten passcode. Building it before then would trade a likely,
recoverable harm for an unlikely, unrecoverable one, and dress it up as
security. Revisit when there is somewhere safe for a key to live.
